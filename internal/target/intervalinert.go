...
func (cm CandidateManager) MarkSideEffects(info *types.Info, body inspector.Cursor) {
	...
	for decl, m := range cm.candidates {
		...
		// The statement itself has no side effects; e.g., x := 5
		if InertStmt(info, node) {
			continue
		}

		// Check for intervening statements with possible side effects
		if intervalInert(info, body, node, m) {
			continue
		}

		m.Status = category.MoveSideEffects
		cm.candidates[decl] = m
	}
}

func intervalInert(info *types.Info, body inspector.Cursor, node ast.Node, m MoveCandidate) bool {
	start, end := node.End(), m.TargetNode.Pos()
	parent, ok := body.FindByPos(start, end)

	return ok && IntervalInert(info, parent, m.AbsorbedDecls, start, end)
}
...
func inertVarDecl(info *types.Info, stmt *ast.DeclStmt) bool {
	...
	for _, spec := range decl.Specs {
		// A ValueSpec with values implies initialization.
