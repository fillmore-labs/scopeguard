...
func TestSafetyCheck(t *testing.T) {
	...
	tests := [...]struct {
		name string
		src  string
		want category.MoveStatus
	}{
		{"simple", `var x int; {_ = x}`, category.MoveAllowed},
		{"declared", `var x int; {x := x; _ = x}`, category.MoveBlockedDeclared},
		{"declared_2", `x := 0; {y := 0; var x = x; _, _ = x, y}`, category.MoveBlockedDeclared},
		{"shadowed", `y := 0; x := y; {y := ""; {_, _ = x, y}}`, category.MoveBlockedShadowed},
		{"not_shadowed", `z := 0; x := z; {y := ""; {_, _, _ = x, y, z}}`, category.MoveAllowed},
		{"redeclaration", `z := 0; {x := z; z := 1; _ = z; {_ = x}}`, category.MoveBlockedShadowed},
		{"no_shadow_after", `z := 0; {x := z; {_ = x}; z := 1; _ = z}`, category.MoveAllowed},
		{"multiple_dependencies", `a, b := 1, 1; x := a + b; {_ = x}`, category.MoveAllowed},
		{"struct_field", `y := struct{f int}{}; x := y.f; {_ = x}`, category.MoveAllowed},
		{"shadowed_struct", `y := struct{f int}{}; x := y.f; {y := 0; {_, _ = x, y}}`, category.MoveBlockedShadowed},
		{"array_index", `y := [1]int{}; x := y[0]; {_ = x}`, category.MoveAllowed},
		{"shadowed_array", `y := [1]int{}; x := y[0]; {y := 0; {_, _ = x, y}}`, category.MoveBlockedShadowed},
	}
