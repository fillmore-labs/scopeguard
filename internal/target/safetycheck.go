...
func SafetyCheck(info *types.Info, decl inspector.Cursor, declScope, targetScope *types.Scope, identifiers iter.Seq[*ast.Ident]) category.MoveStatus {
	// Check if identifiers are already declared in the target scope
	if alreadyDeclaredInScope(targetScope, identifiers) {
		return category.MoveBlockedDeclared
	}

	// Check if moving would cause variables to be shadowed
	if usedIdentifierShadowed(info, decl, declScope, targetScope) {
		return category.MoveBlockedShadowed
	}

	return category.MoveAllowed
}
