...
func TestIntervalInert(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			body, _ := firstBody(t, f)

			blockStmt := body.Node().(*ast.BlockStmt)
			start, end := tt.interval(blockStmt)

			_, info := checkSource(t, fset, []*ast.File{f})

			sc := NewSafetyChecker(info)
			if got, want := sc.IntervalInert(body, nil, start, end), tt.want; got != want {
