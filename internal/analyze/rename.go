...
func (r *Renamer) Renames(info *types.Info, fdecl inspector.Cursor, v *types.Var) []analysis.SuggestedFix {
	...
	for c := range fdecl.Preorder((*ast.Ident)(nil)) {
		...
		if !ok || !identifierIsVar(info, id, v) {
			continue
		}
...
// identifierIsVar checks if the given identifier corresponds to the specified variable.
func identifierIsVar(info *types.Info, id *ast.Ident, v *types.Var) bool {
