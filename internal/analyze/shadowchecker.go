...
type ShadowChecker struct {
	...
	usedAfterShadow UsedAfterShadow
}

// UsedAfterShadow returns the list of variables that were used after being shadowed.
func (sc *ShadowChecker) UsedAfterShadow() UsedAfterShadow {
...
func (sc *ShadowChecker) RecordShadowingDeclaration(scope ScopeIndex, v *types.Var, id *ast.Ident, decl NodeIndex) {
	if sc.shadowed == nil {
		return
	}

	if s, start := scope.Shadowing(v, id.NamePos); s != nil {
...
func (sc *ShadowChecker) RecordShadowedUse(v *types.Var, pos token.Pos, idx NodeIndex) {
...
func (sc *ShadowChecker) recordUsedAfterShadow(v *types.Var, use, decl NodeIndex) {
...
func (sc *ShadowChecker) UpdateShadows(pos, assignmentDone token.Pos, vid assignedVar) {
	...
	// Update shadow end position based on current state:
	// - If no end is set: this is the first reassignment, mark shadow as ending here
	// - If we've passed the end: shadow is done, remove from tracking
	// - If we're before the end: we're in a nested scope (e.g., function literal), update if earlier
	switch hasEnd := s.end.IsValid(); {
	case !hasEnd:
		// First reassignment: set shadowing to end after this assignment
		s.ignore = id.NamePos
		s.end = assignmentDone
		sc.shadowed[v] = s

	case pos >= s.end:
		// We've moved past the shadow end position: shadowing is done
		delete(sc.shadowed, v)

	default:
		// We have an end position but haven't reached it yet in this assignment.
		// This happens in nested scopes (e.g., function literals).
