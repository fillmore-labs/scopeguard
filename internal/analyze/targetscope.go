...
// TargetScope determines where a declaration can be safely moved.
type TargetScope struct {
	ScopeIndex
}

// NewTargetScope creates a new TargetScope instance.
func NewTargetScope(scopes ScopeIndex) TargetScope {
	return TargetScope{ScopeIndex: scopes}
}
...
func (ts TargetScope) FindSafeScope(declScope, minScope *types.Scope) *types.Scope {
	...
	for current := minScope; current != nil; current = ts.parentScope(current) {
		...
		switch ts.ScopeIndex[current].(type) {
		case *ast.ForStmt:
...
func (ts TargetScope) TargetNode(declScope, targetScope *types.Scope, maxPos token.Pos, onlyBlock bool) ast.Node {
	...
	for scope := targetScope; scope != declScope; scope = scope.Parent() {
		...
		targetNode, ok := ts.ScopeIndex[scope]
		if !ok {
