...
func (c *collector) handleFunc(body inspector.Cursor, recv *ast.FieldList, fun *ast.FuncType) {
	...
	if c.ShadowCheckerEnabled() {
		...
		c.Graph = reachability.NewGraph(c.TypesInfo, recv, fun, bodyNode, true)
	}
	...
	current, usage := declUsage{start: start, ignore: token.NoPos}, []DeclarationNode{{Decl: decl, Usage: UsageUsed}}
