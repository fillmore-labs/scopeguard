...
> [!TIP]
>
> For a safer initial run, use `-conservative` with `-fix`. This only applies the changes that don't cross other statements:
>
> ```shell
...
Use `//nolint:scopeguard` at the declaration to suppress this transformation, or better yet, avoid such a complex aliasing
in declarations.
