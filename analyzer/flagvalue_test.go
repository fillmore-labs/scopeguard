...
func TestFlagValue(t *testing.T) {
	...
	tests := []struct {
		name    string
		initial analyze.Analyzer
		args    []string
		want    bool
	}{
		{
			name:    "Enable",
			initial: analyze.ShadowAnalyzer,
			args:    []string{"-scope"},
			want:    true,
		},
		{
			name:    "Disable",
			initial: analyze.ScopeAnalyzer,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			var flags analyze.BitMask[analyze.Analyzer]
			flags.Set(tt.initial, true)

			fs := flag.NewFlagSet("test", flag.ContinueOnError)

			const value = analyze.ScopeAnalyzer
			...
			if flags.Enabled(value) != tt.want {
				t.Errorf("ScopeAnalyzer enabled = %v, want %v", flags.Enabled(analyze.ScopeAnalyzer), tt.want)
			}
		})
	}
}
...
func TestUsage(t *testing.T) {
	...
	var flags analyze.BitMask[analyze.Analyzer]
	flags.Set(analyze.ScopeAnalyzer, true)

	fs := flag.NewFlagSet("test", flag.ContinueOnError)

	fv := NewAnalyzerValue(&flags, analyze.ScopeAnalyzer)
	fs.Var(fv, "scope", "enable scope analyzer")
